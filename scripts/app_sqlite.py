from database_sqlite import extract, transform, load, drop
...
def main():
    ...
    with tab4:
        ...
        **📋 raw_transactions**
        ...
        - All fields preserved from source
        - Used for data audit and reference
        
        **📊 transactions** 
        - Cleaned and processed data
        - Filtered for "Reconciled" status only
        - Used for all analytics and dashboard
        
        ### Key Fields:
        - **type**: Income/Expense classification
        - **date**: Transaction timestamp (DATETIME)
        ...
        ### Database Options:
        - **SQLite**: Local file-based (current)
        - **MySQL**: Server-based (available)
        - **PostgreSQL**: Docker-based (original)
        """)
